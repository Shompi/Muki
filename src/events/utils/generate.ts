...
export const GenerateWelcomeImage = async (member: GuildMember) => {
	...
	/** Username Text */
	context.font = applyText(canvas, `${member.user.username}`, 110, "Anton")
	context.fillStyle = '#eeeeee'
	context.shadowColor = "#000000"
	context.shadowBlur = 2
	context.shadowOffsetX = 1
	context.shadowOffsetY = 1
	context.fillText(`${member.user.username}`, 300, 340)
	/** ----- */

	/** Member count text */
	context.font = applyText(canvas, `Eres el miembro #${member.guild.memberCount}`, 42, "Anton")
	context.fillStyle = '#cacaca'
	context.fillText(`Eres el miembro #${member.guild.memberCount}`, 300, 390)
	/** ----- */
