...
const Command: MessageCommand = {
	...
	async execute(m, args) {
		...
		const context = canvas.getContext('2d')
		const Background = await Canvas.loadImage("./welcome/WelcomeBanner.png")
		...
		/** Member count text */
		context.font = applyText(canvas, `Eres el miembro #${m.guild!.memberCount}`, 58, "Anton")
		context.fillStyle = '#bababa'
		context.fillText(`Eres el miembro #${m.guild!.memberCount}`, 300, 340)
		/** ----- */
